...
@monthMovementsInChart  = Movement.entrada.not_entre_contas.where(payment_date: @date_range).group_by_day(:payment_date).sum(:amount)
    ...
    year_range = Date.new(@year).beginning_of_year..Date.new(@year).end_of_year
    balance_data = Movement.where(payment_date: year_range).group_by_day(:payment_date).sum(:amount)
    
    accumulator = Movement.where('payment_date < ?', year_range.first).sum(:amount)
    @balance_chart = balance_data.transform_values do |val|
      accumulator += val
    end
  ...
  def set_administration
    @employeeMonthSalary = Administration.where('start_date <= ? AND (end_date >= ? Or end_date is null)', @date, @date).where.not(salary: nil).order(:occupation_id, titular: :DESC).all
